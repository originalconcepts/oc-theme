...
<?php else : ?>
	...
	// Logo source: regular (custom_logo) · transparent-state logo · custom upload · none.
	$oc_logo_src = sanitize_html_class( (string) get_theme_mod( 'oc_footer_logo_src', 'regular' ) );
	$oc_logo_url = '';
	if ( 'transparent' === $oc_logo_src ) {
		$oc_logo_url = trim( (string) get_theme_mod( 'oc_logo_transparent', '' ) );
	} elseif ( 'custom' === $oc_logo_src ) {
		$oc_logo_url = trim( (string) get_theme_mod( 'oc_footer_logo_img', '' ) );
	}
	$oc_show_logo = 'none' !== $oc_logo_src;
	...
	// Social profiles come from ONE place — the shop's contact details.
	...
	$oc_socials      = array();
	if ( class_exists( '\OC\Theme\Contact' ) ) {
		foreach ( array( 'instagram', 'facebook', 'tiktok', 'youtube' ) as $oc_key ) {
			$oc_url = \OC\Theme\Contact::get( $oc_key );
			if ( '' !== $oc_url ) {
				$oc_socials[ $oc_key ] = array( $oc_url, $oc_social_icons[ $oc_key ] );
			}
		}
		$oc_wa = preg_replace( '/\D/', '', \OC\Theme\Contact::get( 'whatsapp' ) );
		if ( '' !== (string) $oc_wa ) {
			$oc_socials['whatsapp'] = array( 'https://wa.me/' . $oc_wa, $oc_social_icons['whatsapp'] );
		}
	}
	?>
		...
		<div class="oc-footer__grid">
			...
			<?php if ( $oc_has_brand ) : ?>
				<div class="oc-footer__brand">
					<?php if ( $oc_show_logo ) : ?>
						<?php if ( 'regular' === $oc_logo_src && has_custom_logo() ) : ?>
							<?php the_custom_logo(); ?>
						<?php elseif ( '' !== $oc_logo_url ) : ?>
							<a class="oc-footer__brand-logo" href="<?php echo esc_url( home_url( '/' ) ); ?>"><img src="<?php echo esc_url( $oc_logo_url ); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>" /></a>
						<?php else : ?>
