/**
 * Marketing: one small script that tells the networks what happened.
 *
 * Reads the page's config (window.ocMkt) and the server's event queue
 * (window.ocq), loads the networks after the page has finished, and
 * from then on translates every event once into each dialect — with the
 * same event id the server used, so nothing is counted twice. Nothing
 * here touches jQuery; nothing runs before the page is painted.
 */
( function () {
	'use strict';
	...
	function load() {
		...
		if ( cfg.fb && 'granted' === consent ) {
			...
			/* eslint-enable */
			var match = {};
			[ 'em', 'ph', 'fn', 'ln', 'ct', 'zp', 'country', 'external_id' ].forEach( function ( k ) { if ( adv[ k ] ) { match[ k ] = adv[ k ]; } } );
			window.fbq( 'init', cfg.fb, match );
		...
		if ( cfg.tiktok && 'granted' === consent ) {
			...
			/* eslint-enable */
			if ( adv.em || adv.ph || adv.external_id ) {
				window.ttq.identify( { email: adv.em || undefined, phone_number: adv.ph || undefined, external_id: adv.external_id || undefined } );
			}
		}
	...
	function fbParams( n, d ) {
		...
		if ( it.length ) {
			...
			p.content_ids = it.map( function ( i ) { return String( i.id ); } );
			p.contents = it.map( function ( i ) { return { id: String( i.id ), quantity: i.qty || 1, item_price: Number( i.price || 0 ) }; } );
			p.num_items = it.reduce( function ( a, i ) { return a + ( i.qty || 1 ); }, 0 );
		}
		if ( d.content_name ) { p.content_name = d.content_name; }
	...
	function ttParams( d ) {
		...
		if ( it.length ) {
			p.content_type = 'product';
			p.contents = it.map( function ( i ) { return { content_id: String( i.id ), content_name: i.name || '', quantity: i.qty || 1, price: Number( i.price || 0 ) }; } );
		}
		if ( d.search ) { p.query = d.search; }
		if ( d.order_id ) { p.order_id = d.order_id; }
		return p;
	}

	function gaParams( n, d ) {
		var p = {};
		if ( d.currency ) { p.currency = d.currency; }
		if ( 'undefined' !== typeof d.value ) { p.value = Number( d.value ); }
		...
		if ( 'ViewCategory' === n && d.content_category ) { p.item_list_name = d.content_category; }
		if ( 'ScrollDepth' === n ) { p.percent_scrolled = d.percent; }
		if ( 'VideoStart' === n || 'VideoComplete' === n ) { p.video_url = d.src; }
		var it = items( d );
		if ( it.length ) {
			p.items = it.map( function ( i ) { return { item_id: String( i.id ), item_name: i.name || '', item_category: i.category || undefined, price: Number( i.price || 0 ), quantity: i.qty || 1 }; } );
		}
		return p;
	}
	...
	function send( n, d, id ) {
		...
		window.dataLayer.push( Object.assign( { event: 'oc_' + n, oc_event_id: id }, d ) );

		if ( window.fbq && cfg.fb && 'granted' === consent && 'PageView' !== n ) {
			var fbName = FB[ n ];
			if ( fbName ) { window.fbq( 'track', fbName, fbParams( n, d ), { eventID: id } ); } else if ( 'ViewCategory' === n || 'Login' === n ) { window.fbq( 'trackCustom', n, fbParams( n, d ), { eventID: id } ); }
		}
		...
		if ( cfg.ga4 && GA[ n ] ) {
			gtag( 'event', GA[ n ], Object.assign( gaParams( n, d ), { send_to: cfg.ga4 } ) );
		}

		if ( cfg.gads && 'Purchase' === n && cfg.gadsLabel ) {
			gtag( 'event', 'conversion', { send_to: cfg.gads + '/' + cfg.gadsLabel, value: Number( d.value || 0 ), currency: d.currency, transaction_id: d.order_id } );
		}

		if ( cfg.gads && ( 'ViewContent' === n || 'AddToCart' === n || 'Purchase' === n ) ) {
			var evs = { ViewContent: 'view_item', AddToCart: 'add_to_cart', Purchase: 'purchase' };
			gtag( 'event', evs[ n ], { send_to: cfg.gads, value: Number( d.value || 0 ), items: items( d ).map( function ( i ) { return { id: String( i.id ), google_business_vertical: 'retail' }; } ) } );
		}
	}
	...
	q.forEach( function ( ev ) { send( ev.n, ev.d, ev.id ); } );
	window.ocq = { push: function ( ev ) { send( ev.n, ev.d, ev.id ); } };

	/* ---------- browser-born events ---------- */

	function product( id ) {
		if ( itemCache[ id ] ) { return Promise.resolve( itemCache[ id ] ); }
		if ( cfg.page && cfg.page.item && String( cfg.page.item.id ) === String( id ) ) { return Promise.resolve( cfg.page.item ); }
		return fetch( cfg.rest + 'wc/store/v1/products/' + encodeURIComponent( id ), { credentials: 'same-origin' } )
			.then( function ( r ) { return r.json(); } )
			.then( function ( p ) {
				var div = ( p.prices && p.prices.currency_minor_unit ) ? Math.pow( 10, p.prices.currency_minor_unit ) : 100;
				var item = { id: String( p.id ), name: p.name, price: p.prices ? Number( p.prices.price ) / div : 0, qty: 1, category: ( p.categories && p.categories[ 0 ] ) ? p.categories[ 0 ].name : '' };
				itemCache[ id ] = item;
				return item;
			} )
			.catch( function () { return { id: String( id ), name: '', price: 0, qty: 1, category: '' }; } );
	}

	function addedToCart( id, qty ) {
		if ( ! id ) { return; }
	...
	document.body.addEventListener( 'oc-added-to-cart', function ( e ) { addedToCart( e.detail && e.detail.id, e.detail && e.detail.qty ); } );
	...
	// The product page's own add-to-cart form (single product).
	document.addEventListener( 'submit', function ( e ) {
		var form = e.target.closest( 'form.cart' );
		if ( ! form || ! cfg.page || 'product' !== cfg.page.type ) { return; }
		var qty = parseInt( ( form.querySelector( '[name="quantity"]' ) || {} ).value || '1', 10 ) || 1;
		var vid = ( form.querySelector( '[name="variation_id"]' ) || {} ).value;
		addedToCart( vid && '0' !== vid ? vid : cfg.page.item.id, qty );
	} );

	// Choosing a product from a list.
	document.addEventListener( 'click', function ( e ) {
		var a = e.target.closest( 'li.product a[href]' );
		if ( ! a || ! cfg.ga4 ) { return; }
		var li = a.closest( 'li.product' );
		var id = ( li.querySelector( '[data-product_id]' ) || {} ).dataset;
		id = id && id.product_id;
		var name = ( li.querySelector( '.woocommerce-loop-product__title, .oc-card-title, h2, h3' ) || {} ).textContent;
		gtag( 'event', 'select_item', { send_to: cfg.ga4, items: [ { item_id: String( id || '' ), item_name: ( name || '' ).trim() } ] } );
	} );

	// Payment details, when a method is picked or the order is placed.
	var paid = false;
	...
	if ( cfg.events.scroll ) {
		...
		window.addEventListener( 'scroll', function () {
			if ( ticking || ! marks.length ) { return; }
			ticking = true;
			requestAnimationFrame( function () {
				...
				while ( marks.length && pct >= marks[ 0 ] ) {
					send( 'ScrollDepth', { percent: marks.shift() }, uid( 'scr' ) );
				}
			} );
		}, { passive: true } );
	}

	// Videos: a start and a finish, once each.
	if ( cfg.events.video ) {
		var seen = new WeakMap();
		document.addEventListener( 'play', function ( e ) {
			var v = e.target;
			if ( ! ( v instanceof HTMLVideoElement ) || v.muted && v.autoplay || seen.has( v ) ) { return; }
			seen.set( v, true );
			send( 'VideoStart', { src: v.currentSrc || v.src }, uid( 'vid' ) );
		}, true );
		document.addEventListener( 'ended', function ( e ) {
			var v = e.target;
			if ( ! ( v instanceof HTMLVideoElement ) || v.muted && v.autoplay ) { return; }
			send( 'VideoComplete', { src: v.currentSrc || v.src }, uid( 'vid' ) );
		}, true );
	}

	// TikTok's click id rides the URL once; keep it for the server.
	var m = location.search.match( /[?&]ttclid=([^&]+)/ );
	if ( m ) { setCookie( 'oc_ttclid', decodeURIComponent( m[ 1 ] ), 30 ); }
	...
	if ( 'complete' === document.readyState ) {
		...
		window.addEventListener( 'load', function () {
			( window.requestIdleCallback || function ( f ) { setTimeout( f, 60 ); } )( start );
		} );
	}
}() );
