	...
	public static function build( string $from, string $to, string $kind ): array {
		...
		$subject = 'daily' === $kind
			? sprintf(
				...
				__( '%1$s yesterday: %2$s in %3$d orders', 'oc-theme' ),
				$shop,
			...
			: sprintf(
				/* translators: 1: shop, 2: sales, 3: orders. */
				__( '%1$s this week: %2$s in %3$d orders', 'oc-theme' ),
				$shop,
				self::money( (float) $cur['gross'] ),
				(int) $cur['orders']
			);
		...
		$o .= '<div style="font-size:12px;color:#6b7280;">' . esc_html( $shop ) . ' · ' . esc_html( 'daily' === $kind ? __( 'Daily report', 'oc-theme' ) : __( 'Weekly report', 'oc-theme' ) ) . '</div>';
		$o .= '<div style="font-size:22px;font-weight:700;color:#111827;margin-top:4px;">' . esc_html( $when ) . '</div>';
		$o .= '<div style="font-size:13px;color:#6b7280;margin-top:2px;">' . esc_html( sprintf( /* translators: %s: comparison. */ __( 'Changes are %s.', 'oc-theme' ), $compare ) ) . '</div>';
		$o .= '</td></tr>';
		...
		if ( null !== $week_ago ) {
			$o .= '<tr><td style="padding:2px 24px 10px;font-size:13px;color:#4b5563;">' . esc_html(
				sprintf(
					...
					__( 'Same weekday last week (%1$s): %2$s in %3$d orders.', 'oc-theme' ),
		...
		if ( 'weekly' === $kind ) {
			$o .= '<tr><td style="padding:6px 24px 4px;">' . self::h( __( 'Day by day', 'oc-theme' ) ) . '<table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="font-size:13px;">';
			...
			foreach ( $r['series'] as $s ) {
				...
				$o .= '<td style="padding:5px 0 5px 12px;border-bottom:1px solid #f3f4f6;text-align:' . ( $rtl ? 'left' : 'right' ) . ';color:#6b7280;">' . esc_html( sprintf( /* translators: %d: orders. */ _n( '%d order', '%d orders', (int) $s[2], 'oc-theme' ), (int) $s[2] ) ) . '</td></tr>';
			}
		...
		if ( 'daily' === $kind ) {
			...
			if ( ! empty( $orders ) ) {
				$o .= '<tr><td style="padding:6px 24px 4px;">' . self::h( __( 'The orders', 'oc-theme' ) ) . '<table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="font-size:13px;">';
		...
		if ( (int) $cur['cancelled_n'] > 0 ) {
			$bits[] = sprintf( /* translators: 1: count, 2: sum. */ __( '%1$d cancelled (%2$s)', 'oc-theme' ), (int) $cur['cancelled_n'], self::money( (float) $cur['cancelled_sum'] ) );
		}

		if ( (int) $cur['failed_n'] > 0 ) {
			$bits[] = sprintf( /* translators: 1: count, 2: sum. */ __( '%1$d failed (%2$s)', 'oc-theme' ), (int) $cur['failed_n'], self::money( (float) $cur['failed_sum'] ) );
		}

		if ( (float) $cur['refunds'] > 0 ) {
			$bits[] = sprintf( /* translators: %s: sum. */ __( 'refunds %s', 'oc-theme' ), self::money( (float) $cur['refunds'] ) );
		}

		if ( (int) $cur['pending_n'] > 0 ) {
			$bits[] = sprintf( /* translators: 1: count, 2: sum. */ __( '%1$d awaiting payment (%2$s)', 'oc-theme' ), (int) $cur['pending_n'], self::money( (float) $cur['pending_sum'] ) );
		}

		if ( ! empty( $bits ) ) {
			$o .= '<tr><td style="padding:6px 24px 4px;font-size:13px;color:#4b5563;">' . self::h( __( 'Did not become sales', 'oc-theme' ) ) . esc_html( implode( ' · ', $bits ) ) . '</td></tr>';
		}
		...
		if ( ! empty( $ins ) ) {
			$o .= '<tr><td style="padding:10px 24px 4px;">' . self::h( __( 'Worth doing now', 'oc-theme' ) );
		...
		$o .= '<tr><td style="padding:14px 24px 22px;text-align:center;"><a href="' . esc_url( admin_url( 'admin.php?page=oc-stats' ) ) . '" style="display:inline-block;background:#111827;color:#fff;text-decoration:none;font-size:14px;font-weight:700;padding:11px 20px;border-radius:999px;">' . esc_html__( 'Open the statistics', 'oc-theme' ) . '</a>';
		$o .= '<div style="font-size:11px;color:#9ca3af;margin-top:12px;">' . esc_html__( 'Sales count paid orders and orders in hand, at what the customer paid. Visits are measured by the shop itself; people who declined statistics cookies are not counted.', 'oc-theme' ) . '</div></td></tr>';
		$o .= '</table></div>';
