...
final class Insights {
		...
		foreach ( $win as $slot => $w ) {
			...
			if ( $rate < $avg - 0.15 ) {
				...
				$when = sprintf(
					...
					__( 'on %1$s between %2$02d:00 and %3$02d:00', 'oc-stats' ),
				...
				return array(
					self::one(
						'checkout_drop_' . $slot,
						'crit',
						__( 'Checkout drop', 'oc-stats' ),
						sprintf(
							/* translators: %s: when. */
							__( 'Almost nobody completed the checkout %s', 'oc-stats' ),
							$when
						),
						sprintf(
							/* translators: 1: checkouts, 2: completed, 3: usual rate. */
							__( '%1$d reached the checkout, %2$d completed. The usual rate is %3$d%%. A sign of a payment or shipping failure.', 'oc-stats' ),
							$c,
							$p,
							(int) round( $avg * 100 )
						),
						...
						array(
							array( __( 'Check payment methods', 'oc-stats' ), admin_url( 'admin.php?page=wc-settings&tab=checkout' ) ),
							array( __( 'Failed orders', 'oc-stats' ), admin_url( 'admin.php?page=wc-orders&status=wc-failed' ) ),
						),
						__( 'Rule: in a three-hour window with at least 10 checkouts, completion fell more than 15 points below the 30-day average. Worth: the orders that window would usually have produced, at the average order value.', 'oc-stats' )
					),
				);
			}
		}
	...
	private static function out_of_stock( array $cur, array $prev, array $d90 ): array { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter -- one signature for every rule.
		...
		foreach ( array_slice( $views, 0, 60, true ) as $pid => $n ) {
			...
			$out[] = self::one(
				'stock_' . $pid,
				'crit',
				__( 'Out of stock', 'oc-stats' ),
				$waiting > 0
					? sprintf(
						/* translators: 1: how many wait, 2: product. */
						_n( '%1$d person is waiting for %2$s, and the stock is zero', '%1$d people are waiting for %2$s, and the stock is zero', $waiting, 'oc-stats' ),
						$waiting,
						$product->get_name()
					)
					: sprintf(
						/* translators: %s: product. */
						__( '%s is out of stock and people keep looking at it', 'oc-stats' ),
						$product->get_name()
					),
				sprintf(
					/* translators: 1: views, 2: monthly worth. */
					__( 'Viewed %1$d times in the last 30 days. At its usual pace, restocking is worth about %2$s a month.', 'oc-stats' ),
					(int) $n,
					self::money( $monthly )
				),
				$monthly,
				array_values(
					array_filter(
						array(
							array( __( 'Update stock', 'oc-stats' ), (string) get_edit_post_link( $pid, 'raw' ) ),
							$waiting > 0 ? array( __( 'The waiting list', 'oc-stats' ), admin_url( 'admin.php?page=oc-waitlist' ) ) : null,
						)
					)
				),
				__( 'Rule: at least 40 product views in 30 days, no stock, and optionally people on the back-in-stock list. Worth: the product\'s sales over the last 90 days, divided by three.', 'oc-stats' )
			);
	...
	private static function category_down( array $cur, array $prev, array $d90 ): array { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter -- one signature for every rule.
		...
		foreach ( (array) $prev['cat_gross'] as $tid => $before ) {
			...
			$visits_note = $vprev > 0 && $views >= $vprev * 0.9
				? __( 'Visits to the category held; the drop is inside it — stock that ran out, a price that changed, or a lead product that fell off the first row.', 'oc-stats' )
				: __( 'Visits to the category fell too — fewer people reach it. Worth a place on the home page or in the menu.', 'oc-stats' );

			$out[] = self::one(
				'cat_down_' . $tid,
				'warn',
				__( 'Category', 'oc-stats' ),
				sprintf(
					/* translators: 1: category, 2: percent. */
					__( 'Sales in %1$s fell %2$d%% against the previous 30 days', 'oc-stats' ),
					$term->name,
					(int) round( ( 1 - $now / (float) $before ) * 100 )
				),
				sprintf(
					/* translators: 1: before, 2: now. */
					__( '%1$s before, %2$s now. ', 'oc-stats' ),
					self::money( (float) $before ),
					self::money( $now )
				) . $visits_note,
				(float) $before - $now,
				array(
					array( __( 'Open the category', 'oc-stats' ), (string) get_term_link( $term ) ),
					array( __( 'Its products', 'oc-stats' ), admin_url( 'edit.php?post_type=product&product_cat=' . rawurlencode( $term->slug ) ) ),
				),
				__( 'Rule: a category with at least 1,000 in sales in the previous 30 days whose sales fell by 25% or more. The note reads the category\'s own visits to tell traffic from conversion.', 'oc-stats' )
			);
	...
	private static function buried( array $cur, array $prev, array $d90 ): array { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter -- one signature for every rule.
		...
		foreach ( (array) $cur['product_views'] as $pid => $views ) {
			...
			$out[] = self::one(
				'buried_' . $pid,
				'warn',
				__( 'Catalogue order', 'oc-stats' ),
				sprintf(
					/* translators: 1: product, 2: category. */
					__( '%1$s converts better than the shop, and sits past the first rows of %2$s', 'oc-stats' ),
					$product->get_name(),
					$term->name
				),
				sprintf(
					/* translators: 1: product rate, 2: shop rate, 3: worth. */
					__( 'Its views turn into orders at %1$.1f%% against %2$.1f%% shop-wide. Moving it up, or making it a large tile, is worth about %3$s a month.', 'oc-stats' ),
				...
				$worth,
				array(
					array( __( 'Edit the product', 'oc-stats' ), (string) get_edit_post_link( $pid, 'raw' ) ),
					array( __( 'Open the category', 'oc-stats' ), (string) get_term_link( $term ) ),
				),
				__( 'Rule: at least 100 views and 5 orders in 30 days, a view-to-order rate 40% above the shop\'s, and not among the first 12 products of its category in the shop\'s default order. Worth: 60% of its current monthly sales, the usual lift from the first rows.', 'oc-stats' )
			);
	...
	private static function no_cart( array $cur, array $prev, array $d90 ): array { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter -- one signature for every rule.
		...
		foreach ( array_slice( $views, 0, 40, true ) as $pid => $n ) {
			...
			$out[] = self::one(
				'no_cart_' . $pid,
				'warn',
				__( 'Picture or price', 'oc-stats' ),
				sprintf(
					/* translators: %s: product. */
					__( '%s is viewed a lot, but hardly added to the cart', 'oc-stats' ),
					$product->get_name()
				),
				sprintf(
					/* translators: 1: views, 2: rate, 3: shop rate. */
					__( '%1$d views and %2$.1f%% added to cart, against %3$.1f%% shop-wide. Usually the main photo, the price against competitors, or a thin description.', 'oc-stats' ),
					(int) $n,
					$rate * 100,
					$avg * 100
				),
				...
				array(
					array( __( 'Edit the product', 'oc-stats' ), (string) get_edit_post_link( $pid, 'raw' ) ),
					array( __( 'See it as a shopper', 'oc-stats' ), (string) get_permalink( $pid ) ),
				),
				__( 'Rule: at least 100 views in 30 days and an add-to-cart rate under half the shop\'s. Worth: the adds the shop average would have produced, at the average order value, times the usual 30% that go on to buy.', 'oc-stats' )
			);
	...
	private static function search_misses( array $cur, array $prev, array $d90 ): array { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter -- one signature for every rule.
		...
		foreach ( $rows as $r ) {
			$out[] = self::one(
				...
				'info',
				__( 'Hidden demand', 'oc-stats' ),
				sprintf(
					/* translators: 1: count, 2: term. */
					__( '%1$d searched for "%2$s" this week and found nothing', 'oc-stats' ),
					(int) $r->n,
					(string) $r->term
				),
				__( 'Either the product is missing, or it is called something else in the shop. A synonym fixes the second case in one click.', 'oc-stats' ),
				0,
				array(
					array( __( 'Search reports', 'oc-stats' ), admin_url( 'admin.php?page=oc-search' ) ),
				),
				__( 'Rule: a search term with no results, typed at least 10 times in the last 7 days, from the theme\'s own search log.', 'oc-stats' )
			);
		}
	...
	private static function pairs( array $cur, array $prev, array $d90 ): array { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter -- one signature for every rule.
		...
		foreach ( $count as $k => $n ) {
			...
			$out[] = self::one(
				'pair_' . $k,
				'good',
				__( 'Opportunity', 'oc-stats' ),
				sprintf(
					/* translators: %d: orders. */
					__( 'Two products were bought together in %d orders, and are not a bundle', 'oc-stats' ),
					(int) $n
				),
				sprintf(
					/* translators: 1: product, 2: product, 3: worth. */
					__( '%1$s and %2$s. Offering them as "bought together" with a small discount is worth about %3$s a month.', 'oc-stats' ),
					self::name( $a ),
					self::name( $b ),
					self::money( $worth )
				),
				$worth,
				array(
					array( __( 'Set up the bundle', 'oc-stats' ), (string) get_edit_post_link( $a, 'raw' ) ),
				),
				__( 'Rule: a pair of products in at least 15 paid orders over 30 days, neither listing the other under "bought together". Worth: 10% of the pair\'s sales, the usual lift of a visible bundle.', 'oc-stats' )
			);
	...
	private static function timing( array $cur, array $prev, array $d90 ): array { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter -- one signature for every rule.
		...
		$best      = array( 0, 0, 0.0 );
		$days_name = array( __( 'Sunday', 'oc-stats' ), __( 'Monday', 'oc-stats' ), __( 'Tuesday', 'oc-stats' ), __( 'Wednesday', 'oc-stats' ), __( 'Thursday', 'oc-stats' ), __( 'Friday', 'oc-stats' ), __( 'Saturday', 'oc-stats' ) );
		...
		return array(
			self::one(
				...
				sprintf(
					...
					__( 'Your buyers buy most on %1$s between %2$02d:00 and %3$02d:00', 'oc-stats' ),
					$days_name[ $best[0] ],
					$best[1],
					$best[1] + 3
				),
				sprintf(
					...
					__( 'That window alone holds %s%% of the last 90 days\' orders. A campaign or a message that lands an hour before it works harder.', 'oc-stats' ),
					number_format_i18n( $share, 1 )
				),
				0,
				array(),
				__( 'Rule: paid orders over 90 days, by weekday and hour; the strongest three-hour window.', 'oc-stats' )
			),
		);
	}
}
