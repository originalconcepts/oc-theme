...
class Product {
	...
	public function box(): void {
		...
		add_meta_box( 'oc_stats_product', __( 'Sales', 'oc-stats' ), array( $this, 'render' ), 'product', 'side', 'default' );
	}
	...
	public function render( \WP_Post $post ): void {
		...
		if ( 0 === $all['orders'] ) {
			echo '<p class="ocst-p__none">' . esc_html__( 'This product has not sold yet.', 'oc-stats' ) . '</p>';
			$this->foot();
			return;
		}
		...
		$rows  = array(
			array( __( 'Orders', 'oc-stats' ), number_format_i18n( $m30['orders'] ), number_format_i18n( $all['orders'] ) ),
			array( __( 'Units', 'oc-stats' ), number_format_i18n( $m30['qty'] ), number_format_i18n( $all['qty'] ) ),
			array( __( 'Revenue', 'oc-stats' ), $money( $m30['gross'] ), $money( $all['gross'] ) ),
		);
		?>
		<table class="ocst-p">
			<thead>
				<tr><th></th><th><?php esc_html_e( 'Last 30 days', 'oc-stats' ); ?></th><th><?php echo esc_html( _x( 'All time', 'product sales', 'oc-stats' ) ); ?></th></tr>
			</thead>
			<tbody>
				<?php foreach ( $rows as $r ) : ?>
					<tr><th><?php echo esc_html( $r[0] ); ?></th><td><bdi dir="ltr"><?php echo esc_html( $r[1] ); ?></bdi></td><td><bdi dir="ltr"><?php echo esc_html( $r[2] ); ?></bdi></td></tr>
				<?php endforeach; ?>
			</tbody>
		</table>
		<p class="ocst-p__last">
			<?php esc_html_e( 'Last ordered', 'oc-stats' ); ?>:
			<?php
	...
	private function foot(): void {
		echo '<p class="ocst-p__note">' . esc_html__( 'Paid orders and orders in hand, including VAT; refunds are left out.', 'oc-stats' )
			. ' <a href="' . esc_url( admin_url( 'admin.php?page=' . Admin::PAGE ) ) . '">' . esc_html__( 'All the statistics', 'oc-stats' ) . '</a></p>';
		echo '<style>
