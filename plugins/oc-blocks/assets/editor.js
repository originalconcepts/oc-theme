/**
 * OC Blocks — the composer.
 *
 * One vanilla file: sections as cards on one side, the chosen section's
 * settings on the other, and between them the page itself — rendered by
 * the site at desktop, tablet or phone width. Everything a click or a
 * drag; nothing to learn.
 */
( function () {
	'use strict';

	var D = window.ocBlocks || {};
	var T = D.i18n || {};
	...
	function el( tag, attrs, kids ) {
		...
		Object.keys( attrs || {} ).forEach( function ( key ) {
			...
			} else if ( null !== attrs[ key ] && undefined !== attrs[ key ] ) {
	...
	function fieldsOf( section ) {
		return D.types[ section.type ] ? D.types[ section.type ].fields : {};
	}

	function shown( section, field, fields ) {
		if ( ! field.when ) {
			return true;
		}

		return Object.keys( field.when ).every( function ( on ) {
			var current = section[ on ];

			if ( undefined === current ) {
				current = fields[ on ] ? fields[ on ].def : ( D.shell[ on ] ? D.shell[ on ].def : '' );
			}
	...
	function nameOf( section ) {
		var own = ( section.heading || section.text || '' ).toString().trim();
		var label = D.types[ section.type ] ? D.types[ section.type ].label : section.type;

		return own ? label + ' — ' + own.slice( 0, 24 ) : label;
	}
	...
	function setStatus( text, tone ) {
		if ( els.status ) {
			...
			els.status.className = 'ocbe__status' + ( tone ? ' is-' + tone : '' );
		}
	}

	function schedulePreview() {
		clearTimeout( state.draftTimer );
		state.draftTimer = setTimeout( pushDraft, 450 );
	}

	function pushDraft() {
		var body = new FormData();

		body.append( 'action', 'oc_blocks_draft' );
		body.append( 'nonce', D.nonce );
		body.append( 'sections', JSON.stringify( state.sections ) );

		fetch( D.ajax, { method: 'POST', credentials: 'same-origin', body: body } )
			.then( function ( r ) { return r.json(); } )
			.then( function ( r ) {
				if ( r && r.success ) {
					reloadPreview( r.data.draft );
				}
			} )
			.catch( function () {} );
	}
	...
	function previewUrl( draft ) {
		var glue = D.preview.indexOf( '?' ) > -1 ? '&' : '?';

		return D.preview + glue + 'oc_compose_preview=1&nonce=' + encodeURIComponent( D.nonce ) +
	...
	function save() {
		...
		body.append( 'action', 'oc_blocks_save' );
		body.append( 'nonce', D.nonce );
		body.append( 'page', D.page );
		body.append( 'sections', JSON.stringify( state.sections ) );
		...
		fetch( D.ajax, { method: 'POST', credentials: 'same-origin', body: body } )
			...
			.then( function ( r ) {
				...
				state.sections = r.data.sections;
				state.dirty = false;
	...
	function move( from, to ) {
		if ( null === from || from === to || to < 0 || to >= state.sections.length ) {
			return;
		}

		var picked = state.sections.splice( from, 1 )[ 0 ];

		state.sections.splice( to, 0, picked );

		if ( state.open === from ) {
			state.open = to;
		} else if ( null !== state.open && from < state.open && to >= state.open ) {
			state.open--;
		} else if ( null !== state.open && from > state.open && to <= state.open ) {
			state.open++;
		}
	...
	function card( section, i ) {
		var node = el( 'div', {
			'class': 'ocbe-card' + ( state.open === i ? ' is-on' : '' ) + ( section.on ? '' : ' is-off' ),
			draggable: 'true',
			...
			el( 'button', {
				...
				el( 'span', { 'class': 'ocbe-card__name', text: nameOf( section ) } )
			] ),
			el( 'span', { 'class': 'ocbe-card__tools' }, [
				el( 'button', {
					type: 'button',
					'class': 'ocbe-card__eye' + ( section.on ? '' : ' is-shut' ),
					title: section.on ? T.remove : '',
					'aria-label': 'toggle',
					html: section.on
						? '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6-10-6-10-6z"/><circle cx="12" cy="12" r="2.6"/></svg>'
						: '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 5l16 14M2 12s3.5-6 10-6c1.7 0 3.2.4 4.5 1M22 12s-3.5 6-10 6c-1.7 0-3.2-.4-4.5-1"/></svg>',
					onclick: function ( e ) {
						e.stopPropagation();
						section.on = section.on ? 0 : 1;
						touch();
					}
				} ),
				el( 'button', {
					...
					onclick: function ( e ) {
						...
						if ( state.open === i ) {
							state.open = null;
						} else if ( null !== state.open && state.open > i ) {
							state.open--;
						}
	...
	function adder() {
		var wrap = el( 'div', { 'class': 'ocbe-add' } );
		var list = el( 'div', { 'class': 'ocbe-add__list' } );

		Object.keys( D.types ).forEach( function ( key ) {
			list.appendChild( el( 'button', {
				type: 'button',
				'class': 'ocbe-add__type',
				onclick: function () {
					state.sections.push( blank( key ) );
					state.open = state.sections.length - 1;
					wrap.classList.remove( 'is-open' );
					touch();
					paintSettings();
				}
			}, [
				el( 'span', { 'class': 'ocbe-card__icon', html: D.types[ key ].icon } ),
				el( 'span', {}, [
	...
	function paintRail() {
		...
		if ( ! state.sections.length ) {
			rail.appendChild( el( 'p', { 'class': 'ocbe-rail__empty', text: T.empty } ) );
		}

		state.sections.forEach( function ( section, i ) {
			rail.appendChild( card( section, i ) );
		} );

		rail.appendChild( adder() );
	}

	/* ---------- fields ---------- */

	function fieldRow( section, key, field, fields ) {
		switch ( field.type ) {
			case 'seg':
				return segField( section, key, field );
			case 'toggle':
				return toggleField( section, key, field );
			case 'color':
				return colorField( section, key, field );
			case 'image':
				return imageField( section, key, field );
			case 'video':
				return videoField( section, key, field );
			case 'number':
				return numberField( section, key, field );
			case 'range':
				return rangeField( section, key, field );
			case 'textarea':
	...
	function textField( section, key, field ) {
		var input = el( 'input', {
			type: 'text',
			value: section[ key ] || '',
			oninput: function () {
		...
		return labelWrap( field, [ input ] );
	}

	function areaField( section, key, field ) {
		...
		return labelWrap( field, [ input ], true );
	}

	function numberField( section, key, field ) {
		var input = el( 'input', {
			type: 'number',
			min: undefined !== field.min ? String( field.min ) : null,
			max: undefined !== field.max ? String( field.max ) : null,
			value: undefined !== section[ key ] && '' !== section[ key ] ? section[ key ] : ( field.def || 0 ),
			oninput: function () {
	...
	function rangeField( section, key, field ) {
		...
		return labelWrap( field, [ el( 'span', { 'class': 'ocbe-f__range' }, [ input, shownV ] ) ], true );
	}

	function toggleField( section, key, field ) {
		var input = el( 'input', {
			type: 'checkbox',
			checked: section[ key ] ? 'checked' : null,
			onchange: function () {
				section[ key ] = input.checked ? 1 : 0;
				touch();
				paintSettings();
			}
		} );

		return el( 'label', { 'class': 'ocbe-f ocbe-f--toggle' }, [
			el( 'span', { 'class': 'ocbe-f__l', text: field.label } ),
			el( 'span', { 'class': 'ocbe-switch' }, [ input, el( 'i' ) ] ),
			field.hint ? el( 'small', { 'class': 'ocbe-f__hint', text: field.hint } ) : null
		] );
	}
	...
	function segField( section, key, field ) {
		...
		return labelWrap( field, [ wrap ], Object.keys( field.choices ).length > 3 );
	}

	function colorField( section, key, field ) {
		var input = el( 'input', {
			type: 'color',
			value: section[ key ] || '#111111',
			oninput: function () {
				section[ key ] = input.value;
				touch();
			}
		...
		return labelWrap( field, [ el( 'span', { 'class': 'ocbe-f__color' }, [ input, section[ key ] ? clear : null ] ) ] );
	}

	function imageField( section, key, field ) {
		var id = Number( section[ key ] || 0 );
		var box = el( 'button', {
			type: 'button',
			'class': 'ocbe-pic' + ( id ? ' has-pic' : '' ),
			onclick: function () {
		...
		if ( id && state.thumbs[ id ] ) {
			box.style.backgroundImage = 'url(' + state.thumbs[ id ] + ')';
		} else {
			box.innerHTML = '<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="9" cy="10" r="2"/><path d="M4 18l5-5 4 4 3-3 4 4"/></svg><span>' + ( T.pick ) + '</span>';
		}

		var clear = id ? el( 'button', {
			type: 'button',
			'class': 'ocbe-f__clear',
			text: T.clear,
			onclick: function () {
		...
		return labelWrap( field, [ el( 'span', { 'class': 'ocbe-pic__row' }, [ box, clear ] ) ], true );
	}

	function videoField( section, key, field ) {
		var input = el( 'input', {
			type: 'url',
	...
	function categoryField( section, key, field ) {
		var select = el( 'select', {
			onchange: function () {
				section[ key ] = Number( select.value );
				touch();
			}
		} );

		select.appendChild( el( 'option', { value: '0', text: '—' } ) );

		treeWalk( 0, 0, function ( cat, depth ) {
			select.appendChild( el( 'option', {
				value: String( cat.id ),
				selected: Number( section[ key ] ) === cat.id ? 'selected' : null,
				text: new Array( depth + 1 ).join( ' ' ) + cat.label
			} ) );
		} );
	...
	function treeWalk( parent, depth, fn ) {
	...
	function catsField( section, key, field ) {
		...
		return labelWrap( field, [ list ], true );
	}

	function picksField( section, key, field ) {
		var wrap = el( 'div', { 'class': 'ocbe-picks' } );
		var chips = el( 'div', { 'class': 'ocbe-chips' } );

		function paintChips() {
			chips.innerHTML = '';

			( section[ key ] || [] ).forEach( function ( id, at ) {
				chips.appendChild( el( 'span', { 'class': 'ocbe-chip' }, [
					el( 'span', { text: state.names[ id ] || ( '#' + id ) } ),
					el( 'button', {
						type: 'button',
						html: '&times;',
						onclick: function () {
		...
		box.addEventListener( 'input', function () {
			...
			timer = setTimeout( function () {
				...
				body.append( 'nonce', D.nonce );
				body.append( 'kind', 'posts' === field.type ? 'posts' : 'product' );
				body.append( 'q', q );
						...
						r.data.hits.forEach( function ( hit ) {
							drop.appendChild( el( 'button', {
								...
								onclick: function () {
									...
									if ( section[ key ].map( Number ).indexOf( hit.id ) < 0 ) {
										section[ key ].push( hit.id );
	...
	function slidesField( section, key, field ) {
		...
		( section[ key ] || [] ).forEach( function ( slide, at ) {
			...
			var tools = el( 'span', { 'class': 'ocbe-slide__tools' }, [
				at > 0 ? el( 'button', {
					type: 'button', html: '&uarr;', 'aria-label': 'up',
					onclick: function () {
						section[ key ].splice( at - 1, 0, section[ key ].splice( at, 1 )[ 0 ] );
						touch();
						paintSettings();
					}
				} ) : null,
				at < section[ key ].length - 1 ? el( 'button', {
					type: 'button', html: '&darr;', 'aria-label': 'down',
					onclick: function () {
						section[ key ].splice( at + 1, 0, section[ key ].splice( at, 1 )[ 0 ] );
						touch();
						paintSettings();
				...
				el( 'button', {
					type: 'button', html: '&times;', 'aria-label': T.remove,
					onclick: function () {
						section[ key ].splice( at, 1 );
						touch();
						paintSettings();
			...
			wrap.appendChild( el( 'div', { 'class': 'ocbe-slide' }, [
				el( 'div', { 'class': 'ocbe-slide__bar' }, [ head, tools ] ),
				body
			] ) );
		} );
	...
	function group( title, open ) {
		var body = el( 'div', { 'class': 'ocbe-group__body' } );
		var head = el( 'button', {
			type: 'button',
			'class': 'ocbe-group__head',
			onclick: function () {
				box.classList.toggle( 'is-open' );
			}
		}, [
			el( 'span', { text: title } ),
			el( 'span', { 'class': 'ocbe-group__chev', html: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>' } )
		] );
		var box = el( 'div', { 'class': 'ocbe-group' + ( open ? ' is-open' : '' ) }, [ head, body ] );

		return { box: box, body: body };
	}

	function paintSettings() {
		var pane = els.settings;
		...
		var section = state.sections[ state.open ];
		var fields = fieldsOf( section );

		pane.appendChild( el( 'h2', { 'class': 'ocbe-side__title', text: D.types[ section.type ].label } ) );

		var content = group( T.content, true );
		...
		Object.keys( D.shell ).forEach( function ( key ) {
			if ( 'on' === key || ! shown( section, D.shell[ key ], D.shell ) ) {
				return;
			}

			shell.body.appendChild( fieldRow( section, key, D.shell[ key ], D.shell ) );
		} );

		if ( content.body.children.length ) {
			pane.appendChild( content.box );
		}

		if ( design.body.children.length ) {
			pane.appendChild( design.box );
		}

		pane.appendChild( shell.box );

		pane.appendChild( el( 'button', {
			type: 'button',
			'class': 'ocbe-side__dup',
			text: T.dup,
			onclick: function () {
				state.sections.splice( state.open + 1, 0, JSON.parse( JSON.stringify( section ) ) );
				state.open++;
				touch();
				paintSettings();
			}
	...
	var DEVICES = { desktop: 1280, tablet: 810, mobile: 400 };

	function setDevice( name ) {
		state.device = name;

		els.deviceBtns.forEach( function ( btn ) {
			btn.classList.toggle( 'is-on', btn.dataset.dev === name );
		} );
	...
	function build() {
		var root = document.getElementById( 'oc-blocks-root' );

		if ( ! root ) {
			return;
		}

		document.body.classList.add( 'ocbe-full' );
		...
		var deviceIcons = {
			desktop: '<svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="2.5" y="4" width="19" height="13" rx="1.5"/><path d="M9 21h6M12 17.5V21"/></svg>',
			tablet: '<svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="5" y="2.5" width="14" height="19" rx="2"/><path d="M11 18.5h2"/></svg>',
			mobile: '<svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="7" y="2.5" width="10" height="19" rx="2"/><path d="M11 18.5h2"/></svg>'
		};

		els.deviceBtns = Object.keys( DEVICES ).map( function ( name ) {
			return el( 'button', {
				type: 'button',
				'class': 'ocbe-dev__b' + ( state.device === name ? ' is-on' : '' ),
				'data-dev': name,
				'aria-label': name,
				html: deviceIcons[ name ],
				onclick: function () {
					setDevice( name );
				}
			} );
		...
		var header = el( 'header', { 'class': 'ocbe-top' }, [
			...
			el( 'span', { 'class': 'ocbe-top__end' }, [
				...
				el( 'button', { type: 'button', 'class': 'ocbe-top__save', text: T.save, onclick: save } )
			] )
		] );

		root.appendChild( header );
		root.appendChild( el( 'div', { 'class': 'ocbe-body' }, [
			el( 'aside', { 'class': 'ocbe-rail' }, [
				el( 'h2', { 'class': 'ocbe-rail__title', text: T.sections } ),
				els.rail
			] ),
			els.stage,
			el( 'aside', { 'class': 'ocbe-side' }, [ els.settings ] )
		] ) );
